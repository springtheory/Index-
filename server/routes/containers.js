...
// Get all containers
router.get('/', (req, res) => {
  const containers = inventory.getAllContainers();
  res.json(containers);
});

// Get single container with items
router.get('/:id', (req, res) => {
  const container = inventory.getContainer(req.params.id);
...
router.post('/', (req, res) => {
  ...
  const container = inventory.createContainer({ label, type, color, size, description, location_id });
